...
# 创建结果保存目录
result_dir = f"PSO_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
...
for run in range(10):
    ...
    try:
        ...
        plt.plot(convergence_data, 'g-.', linewidth=2)  # 修改曲线样式
        plt.xlabel('Iterations', fontsize=12)
        plt.ylabel('Fitness', fontsize=12)
        plt.title(f'PSO Convergence - Run {run + 1}', fontsize=14)  # 修改标题
...
df_summary.to_csv(f"{result_dir}/experiment_summary_PSO.csv", index=False)
