...
# 创建结果保存目录
result_dir = f"BA_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
...
for run in range(10):
    ...
    try:
        ...
        # 模型训练与评估（使用选择的特征）
        mdl = KNeighborsClassifier(n_neighbors=opts['k'])  # 使用参数中的k值 ★关键修复★
...
df_summary.to_csv(f"{result_dir}/experiment_summary_BA.csv", index=False)
