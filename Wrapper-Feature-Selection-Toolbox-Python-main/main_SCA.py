...
# 创建结果保存目录
result_dir = f"SCA_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
...
for run in range(10):
    ...
    try:
        ...
        plt.plot(convergence_data, color='darkorange', linewidth=2, linestyle='-')
        plt.xlabel('Iterations', fontsize=12)
        plt.ylabel('Fitness Value', fontsize=12)
        plt.title(f'SCA Convergence Process - Run {run + 1}', fontsize=14)
...
df_summary.to_csv(f"{result_dir}/experiment_summary_SCA.csv", index=False)
