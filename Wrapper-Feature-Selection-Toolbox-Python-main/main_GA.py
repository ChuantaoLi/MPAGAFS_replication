...
# 创建结果保存目录
result_dir = f"GA_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
...
for run in range(10):
    ...
    try:
        ...
        # 参数设置
        opts = {'k': 5, 'fold': fold, 'N': 30, 'T': 50, 'CR': 0.7, 'MR':0.05}
...
df_summary.to_csv(f"{result_dir}/experiment_summary_GA.csv", index=False)
